...
## B11 — Situation showcase (cards + explainability gallery)

Presentation-quality renderings of B8/B8b naming + B5/B7b intervention
anatomy + B8.3 faithfulness, in two forms generated by one script:
**static PNG + vector PDF** (paper/thesis) and **one self-contained
interactive HTML page** (`showcase.html`, supervisor-shareable).
NO new experiments — all data from existing artefacts.

### Two-level naming (the "evocative" request, made defensible)

`config/situation_label_dictionary.json` is the version-controlled
mapping `(time_token, intent_token) → readable_label`. The technical
name from B8b (deterministic from the centroid) is the **subtitle**;
the dictionary-derived readable label is the **prominent title**.
Anything not in the dictionary falls back to the technical name (no
hand-invented poetry). Distinctness across cards is enforced by a
deterministic ` #<id>` suffix on collisions.

**NYC mask** (8 situations):

| id | readable | technical | sink |
|---|---|---|---|
| s0 | *Morning stroll #0* | Morning · Outdoors (+Food) @08h #0 | |
| s1 | *(fallback)* | Evening · Mixed | |
| s2 | *Afternoon stroll* | Afternoon · Outdoors | |
| s3 | *(fallback)* | Afternoon · Mixed (+Shop) | |
| s4 | *(fallback)* | Morning · Mixed (+Shop) | |
| s5 | *(fallback)* | Afternoon · Mixed (+Outdoors) | |
| s6 | **Morning stroll #6** | Morning · Outdoors (+Food) @08h #6 | ★ |
| s7 | *(fallback)* | Morning · Mixed (+Outdoors) | ★ |

**TKY keep** (6 situations):

| id | readable | technical | sink |
|---|---|---|---|
| s0 | *Weekday hop* | Weekday Afternoon · Transit | |
| s1 | *Weekend trip* | Weekend Afternoon · Transit | |
| s2 | *Morning commute #2* | Morning · Transit (+Shop) @08h #2 | |
| s3 | *Morning commute #3* | Morning · Transit (+Shop) @08h #3 | |
| s4 | **Afternoon hop** | Afternoon · Transit | ★ |
| s5 | **Weekday commute (PM)** | Weekday Evening · Transit | ★ |

The "Mixed" situations on NYC keep the fallback honestly — the cards
*show* their full intent composition as ranked bars (no bare "Mixed"
label hidden behind nothing) so the diffuse character is visible at a
glance.

### Card structure (`outputs/round3/B11/<city>/cards/s<k>.{png,pdf}`)

Each card has:
1. Header with readable title + technical subtitle + sink badge if
   applicable.
2. Three stat tiles: frequency (% of test), certainty (% core in test),
   KL × global (red if > 1.5 — sink visual cue).
3. Signature row: modal hour vs city modal hour; weekend share vs city.
4. **Full intent composition** as ranked bars with percentages (top
   attractor red on sinks).
5. Top next-venue macros from the test set as chips.

### Explainability panels (`outputs/round3/B11/<city>/explainability/s<k>.{png,pdf}`)

One per situation. For sink situations: a representative touched test
request, side-by-side B_blind vs X-SAGE top-5 with the entering long-
tail items highlighted (★), score decomposition `score_total =
score_base + nudge`, the natural-language explanation citing the
readable name, and a faithfulness foot citing B8.3 (0/76 934
violations). For non-sink situations: the same panel structure, but
the two lists are identical, the nudge is 0, and the explanation
honestly states "this situation is not an inequity sink; X-SAGE list =
B_blind list".

### Interactive HTML (`outputs/round3/B11/showcase.html`)

44 KB self-contained file. Open as `file://` — no server, no external
network. Two-city tabs, card grid; click a card to expand its
explainability panel. Same numbers as the static figures (both
generated from one assembly step). Dark-mode-safe palette.

### Outputs (B11)

* `outputs/round3/B11/<city>/cards/s<k>.{png,pdf}` — 14 cards
* `outputs/round3/B11/<city>/explainability/s<k>.{png,pdf}` — 14 panels
* `outputs/round3/B11/<city>/{cards_data.json, explainability_data.json}`
* `outputs/round3/B11/showcase.html` (supervisor-shareable)
* `config/situation_label_dictionary.json` (versioned label mapping)
...
| B11   | situation showcase | 14 cards + 14 explainability panels (PNG+PDF) + `showcase.html` self-contained; dictionary-driven readable labels + technical subtitles | **MAIN figures** (card grid + explainability gallery; `showcase.html` for supervisors) |
