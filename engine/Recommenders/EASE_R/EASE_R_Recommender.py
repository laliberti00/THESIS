...
from engine.Recommenders.Recommender_utils import similarityMatrixTopK, check_matrix
...
class EASE_R_Recommender(BaseItemSimilarityMatrixRecommender):
    ...
    def _compute_score_W_dense(self, user_id_array, items_to_compute = None):
        ...
        if items_to_compute is not None:
            item_scores = - np.ones((len(user_id_array), self.URM_train.shape[1]), dtype=float)*np.inf
