...
from engine.Data_manager.IncrementalSparseMatrix import IncrementalSparseMatrix
...
def split_train_test_validation(loaded_dataset, test_data_dictionary, validation = False, validation_portion = 0.1):
    ...
    # Create test data csr matrix by using the testing dictionary provided by the original authors 
    # Create training csr matrix by using the training dictionary provided by the original authors
    updated_test_data_dict = update_item_ids_of_original_data(loaded_dataset.item_original_ID_to_index,loaded_dataset.user_original_ID_to_index, test_data_dictionary)
    ...
    if use_validation_set:  # if use_validation_set is true then create training validation data and testing validation data.
    ...
    for user_id in range(URM.shape[0]):
        ...
        if (user_id in updated_test_data_dict):
            test_records_items = np.array(list(updated_test_data_dict[int(user_id)]))
            test_data = np.ones(len(test_records_items))
            
            start_user_position = URM.indptr[user_id]
            end_user_position = URM.indptr[user_id+1]
            
            # all items of a user from the provided train and test splits
            ...
            URM_train_builder.add_data_lists([user_id]*len(user_interaction_items), user_interaction_items, user_interaction_data)
    ...
    # create training and testing validation splits from the URM_train
    if use_validation_set == True:
        ...
        for user_id in range(URM_train.shape[0]):
            ...
            if len(user_profile) > 0 and k_out > 0:
                ...
                # users interactions for test validation data......
                ...
                # users interactions for training validation......
